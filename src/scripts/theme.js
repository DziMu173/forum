...
if (dropdownButton) {
  switch (sort) {
    ...
    case 'desc':
      ...
      break;
    case 'id':
      dropdownButton.textContent = 'По дате создания';
      break;
    default:
      dropdownButton.textContent = 'Сортировка';
      break;
  }
}
      ...
      await fetch('api/thread/create', {
        ...
        .then((response) => {
          ...
          if (isCreatedThread.isCreatedThread === true) {
            ...
            message('Тема уже существует!', 'notification');
          }
        });
      } else {
        message('Ошибка! Пожалуйста, проверьте правильность заполнения поля.', 'notification');
      }
  }));
}
